...
function unoGame(config){
    ...
    async function processAiTurns(message, game){
        ...
        while (nextPlayer.isAi){
            ...
            await botBrain.play(nextPlayer.hand, game.currentCard, 
                    ...
                    (playerName) => aiDraw(message, game, playerName));
            
            nextPlayer = game.players[game.turnOrder[0]];
        }
    };
    
    async function aiPlay(message, game, color, value, aiPlayerName){
        ...
        await sendMessage(message, aiPlayerName + ' would have played a ' + color + ' ' + value);
        await this.announceTurn(message, game);
        
        return 'play';
    };
    
    async function aiDraw(message, game, aiPlayerName){
