...
function unoGame(config){
    ...
    function processAiTurns(message, game){
        var nextPlayer = game.players[game.turnOrder[0]];
        
        while (nextPlayer.isAi){
            var botBrain = require('../ai/' + nextPlayer.aiName + '.ai.js')();
            var playAiTurn = function(){
                botBrain.play(nextPlayer.hand, game.currentCard, 
                    (color, value, playerName) => aiPlay(message, game, color, value, playerName), 
                    function(playerName){
                        aiDraw(message, game, playerName).then(() => playAiTurn());
                    });
            };
    ...
    function aiPlay(message, game, color, value, aiPlayerName){
        //Check value for wild
        //  interpret color as desired color
        //Ensure the AI has the selected card
        //Remove the card from the AI hand
        //Perform any special actions
        //End AI turn
        //Save game
        //Play next AI turn if aplicable
        
        endTurn(message, game);
        
        console.log('AI played: ' + color + ' ' + value);
        
        return saveGame(game).then(function(){
            sendMessage(message, aiPlayerName + ' would have played a ' + color + ' ' + value);
            this.announceTurn(message, game);
        });
    }
    
    function aiDraw(message, game, aiPlayerName){
        ...
        return drawCards(message, game, aiPlayerName, 1).then(function(){
            sendMessage(message, aiPlayerName + ' has drawn a card.');
        }).then(function(){
            return saveGame(game).then(function(){
                this.announceTurn(message, game);
            });
