...
function unoGame(config){
    ...
    this.addAiPlayer = function(message, game, aiName, botName){
        ...
        if (!brain.develop){
            game.players[botName].play = brain.play;
        } 
    ...
    this.playCard = function(message, game, color, value){
        ...
        Promise.all(asyncs).then(function(){
            saveGame(game).then(function(){
                ...
                if (nextPlayer.isAi){
                    //Will have a 'play' callback if the ai is not in development.
                    //Otherwise, construct the current AI and execute it.
                    if (nextPlayer.play){
                        nextPlayer.play(nextPlayer.hand, game.currentCard, aiPlay, aiDraw);
                    } else {
                        var brain = require('../ai/' + nextPlayer.aiName + '.ai.js')();
                        brain.play(nextPlayer.hand, game.currentCard, aiPlay, aiDraw);
                    }
                }
            });
        });
