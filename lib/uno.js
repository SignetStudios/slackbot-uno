...
function unoGame(config){
    ...
    var endGame = async function(message, game){
        ...
        if (currentScores[0].Score >= 500){
            ...
            await sendMessage(message, gameWinner.Name + ' has won the game with ' + gameWinner.Score + ' points!', true);
            ...
            await sendMessage(message, game.player1 + ', type `/uno start` to begin a new hand.', true);
        }
    ...
    this.addAiPlayer = async function(message, game, aiName, botName){
        ...
        if (game.turnOrder.indexOf(botName) > 0){
            await sendMessage(message, 'There is already a player named ' + botName + ' in the game!', true);
            return;
        }
    ...
    this.playCard = async function(message, game, color, value){
        ...
        if (!color && !value){
            await this.reportHand(message, game);
            await sendMessage(message, 'You can perform the following actions:\n`/uno play [card]`, `/uno draw`, `/uno view`', true);
            return;
        }
    ...
    this.quitGame = async function(message, game){
        var user = message.body.user_name;
        ...
        if (!game.players[user]){
            await sendMessage(message, 'You weren\'t playing to begin with.', true);
            return;
        }
    ...
    this.reportHand = async function(message, game){
        if (!game){
            return;
        }
    ...
    this.setWildColor = async function(message, game, color){
        if (message.type !== 'action'){
            await sendMessage(message, 'Please select a color.', true);
            message.route('colorSelection', game);
            return;
        }
