...
function unoGame(config){
    ...
    this.beginTurnInteractive = async function(message, game){
        ...
        for (var color in colors){
            var handColors = hand.filter(function(item){ return item.color === colors[color]; });
            ...
            for (var card in handColors){
                ...
                attachment.actions.push({
                    "name": handColors[card].color + ' ' + handColors[card].value,
                    "text": colors[color] + ' ' + handColors[card].value,
                    "type": "button",
                    "value": handColors[card].color + ' ' + handColors[card].value
                });
            }
        }
        ...
        */
        ...
        -Build message
            -Current up card
            -Each card in hand (split by color, w/ action to play)
            -Draw action, Status/Score action
        -Display message
        -Route to action
        */
